...
import math
import cv2 as cv
#依次为读取的图片文件夹路径，读取的标签文件夹路径，生成的图片文件夹路径和生成的标签文件夹路径
path = 'C:/programing/faster-CTPN-master/data/traindata/img'
gt_path = 'C:/programing/faster-CTPN-master/data/traindata/label'
out_path = 'C:/programing/faster-CTPN-master/data/traindata/re_img'
label_path = 'C:/programing/faster-CTPN-master/data/traindata/label_temp'
...
for file in files:
    ...
    #灰度化处理
    re_im = cv.cvtColor(re_im,cv.COLOR_RGB2GRAY)
    re_im = cv.cvtColor(re_im,cv.COLOR_GRAY2RGB)
    re_size = re_im.shape
    ...
    for line in lines:
        ...
        with open(os.path.join(label_path, stem) + '.txt', 'w') as f:
