...
class Network(object):
    ...
    #modify_ctpn
    @layer
    def conv1D(self, input, filters, kernel_size, strides, name, padding='same', activation='relu', trainable=True):
        img = input

        shape = tf.shape(img)
        N, H, W, C = shape[0], shape[1], shape[2], shape[3]
        img = tf.reshape(img, [N*H, W, C])
        img.set_shape([None, None, 512])

        #regularizer = tf.contrib.layl2_regularizer(scale=0.00001)

        outputs = img
        for i in range(2):
            #a conv1d-bn-relu block
            outputs = tf.layers.conv1d(
                ...
                kernel_regularizer=self.l2_regularizer(cfg.TRAIN.WEIGHT_DECAY),
                name=name+"%s"%i
            )

            outputs = tf.layers.batch_normalization(
                                outputs,
                                gamma_regularizer = self.l2_regularizer(0.00001),
                                beta_regularizer = self.l2_regularizer(0.00001)
                             )
            outputs = tf.nn.relu(outputs)

        outputs = tf.reshape(outputs, [N, H, W, 512])

        return outputs

    @layer
    def conv1d_fc(self, input, d_i, d_o, name, trainable=True):
    ...
    def build_output_imgage(self,ohem=False):
#         print('input',self.get_output('data').shape.as_list())
#         print('conv1_2',self.get_output('conv1_2').shape.as_list())
#         print('conv2_2',self.get_output('conv2_2').shape.as_list())
#         print('conv3_3',self.get_output('conv3_3').shape.as_list())
#         print('conv4_3',self.get_output('conv4_3').shape.as_list())
#         print('conv5_3',self.get_output('conv5_3').shape.as_list())
#         print('rpn_conv/3x3',self.get_output('rpn_conv/3x3').shape.as_list())
#         print('rpn_cls_score',self.get_output('rpn_cls_score').shape.as_list())
#         print('rpn_bbox_pred',self.get_output('rpn_bbox_pred').shape.as_list())
        input_img = self.get_output('data')
        conv1_2_img = tf.transpose(self.get_output('conv1_2'),[3,1,2,0])
        conv2_2_img = tf.transpose(self.get_output('conv2_2'),[3,1,2,0]) 
        conv3_3_img = tf.transpose(self.get_output('conv3_3'),[3,1,2,0]) 
        conv4_3_img = tf.transpose(self.get_output('conv4_3'),[3,1,2,0])   
        conv5_3_img = tf.transpose(self.get_output('conv5_3'),[3,1,2,0]) 
        rpn_conv_img = tf.transpose(self.get_output('rpn_conv/3x3'),[3,1,2,0]) 
#         reshape_img = tf.transpose(self.get_output('Reshape'),[3,1,2,0])
#         reshape_1_img = tf.transpose(self.get_output('Reshape_1'),[3,1,2,0])
#         reshape_2_img = tf.transpose(self.get_output('Reshape_2'),[3,1,2,0])
#         reshape_3_img = tf.transpose(self.get_output('Reshape_3'),[3,1,2,0])     
#         rpn_cls_score_img = tf.transpose(self.get_output('rpn_cls_score'),[3,1,2,0]) 
#         rpn_bbox_pred_img = tf.transpose(self.get_output('rpn_bbox_pred'),[3,1,2,0]) 

        return input_img,conv1_2_img,conv2_2_img,conv3_3_img,conv4_3_img,conv5_3_img,rpn_conv_img
